<template>
  <div class="flex flex-col h-full">
    ...
    <Teleport v-if="toolbarReady" to="#app-toolbar-slot">
      <div class="flex items-center gap-2 h-full px-3 overflow-x-auto scrollbar-none flex-1 min-w-0">
        ...
        <template v-if="browserStore.selectedFolder && !browserStore.loading && browserStore.files.length > 0">
          ...
          <!-- 마법사 -->
          <div class="relative shrink-0" ref="wizardMenuRef">
            <button
              class="btn-toolbar !bg-indigo-100 !text-indigo-700 hover:!bg-indigo-200 dark:!bg-indigo-900/30 dark:!text-indigo-400 flex items-center gap-1"
              :title="t('wizard.title')"
              @click="toggleWizardMenu"
            >🪄 {{ t('wizard.title') }}<span class="text-[10px] opacity-60">▾</span></button>
            <div
              v-if="showWizardMenu"
              class="fixed top-10 w-56 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl z-[200] py-1.5 overflow-y-auto max-h-72"
              :style="wizardMenuPos"
            >
              <!-- 프리셋 목록 -->
              <template v-if="wizardSavedPresets.length > 0">
                <p class="px-3 py-1 text-[10px] font-semibold text-gray-400 uppercase tracking-wider">{{ t('wizard.preset.selectTitle') }}</p>
                <button
                  v-for="preset in wizardSavedPresets"
                  :key="preset.id"
                  class="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors text-left"
                  @click="openWizardWithPreset(preset)"
                >
              ...
              <!-- 기본 옵션 -->
              <button
                class="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-left"
                @click="openWizardSetup"
              >
                <span class="text-base">⚙️</span>
                <span>{{ t('wizard.preset.openSetup') }}</span>
              </button>
            </div>
          </div>
        </template>
    ...
    <div class="relative flex-1 flex overflow-hidden min-h-0">
      ...
      >
        ...
        <template v-else>
          ...
          <!-- ── 모바일 카드 뷰 (md 미만) ── -->
          <div v-if="browserStore.displayFiles.length > 0" class="md:hidden">
            <!-- 그룹 모드 (하위폴더 포함 로드) -->
            <template v-if="browserStore.displayGroups.length > 0">
              <template v-for="group in browserStore.displayGroups" :key="group.folder_path">
                <div class="px-4 py-1.5 bg-gray-50 dark:bg-gray-800/80 border-y border-gray-200 dark:border-gray-700/60 flex items-center gap-2 select-none">
                  <span class="text-yellow-400 text-xs shrink-0">📁</span>
                  <span class="text-xs font-semibold text-gray-600 dark:text-gray-300 truncate">{{ group.relative_path || group.folder_name }}</span>
          ...
          <table class="w-full min-w-[1400px] text-sm">
              ...
              <!-- 그룹 모드 (하위폴더 포함 로드) -->
              <template v-if="browserStore.displayGroups.length > 0">
                <tbody v-for="group in browserStore.displayGroups" :key="group.folder_path">
                  <tr class="bg-gray-50 dark:bg-gray-800/80 border-y border-gray-200 dark:border-gray-700/60 select-none">
                    <td colspan="18" class="px-4 py-1.5">
                      <div class="flex items-center gap-2">
                        <span class="text-yellow-400 text-xs shrink-0">📁</span>
                        <span class="text-xs font-semibold text-gray-600 dark:text-gray-300">{{ group.relative_path || group.folder_name }}</span>
                        <span class="text-[10px] text-gray-400">{{ group.files.length }}곡</span>
                      </div>
                    </td>
                  </tr>
                  <tr
                    v-for="file in group.files"
                    :key="file.path"
                    class="group border-t border-gray-100 dark:border-gray-800 transition-colors"
                    :class="[
                      file.is_cue_track ? 'cursor-default' : 'cursor-pointer',
                      browserStore.selectedFile?.path === file.path
                        ...
                        : file.is_cue_track
                        ? 'bg-gray-50/50 dark:bg-gray-800/20 border-l-2 border-transparent'
                  ...
                  >
                    ...
                    <td class="px-2 py-2 max-w-[200px]">
                      <div class="flex items-center gap-1.5">
                        ...
                        <span v-if="file.is_title_track" class="shrink-0 text-[9px] font-extrabold px-1.5 py-0.5 rounded bg-gradient-to-r from-orange-500 to-red-500 text-white leading-none">{{ t('browser.titleBadge') }}</span>
                      </div>
                    </td>
                    <td class="px-3 py-2 text-gray-600 dark:text-gray-400 max-w-[130px]"><p class="truncate">{{ file.artist }}</p></td>
                    <td class="px-3 py-2 text-gray-600 dark:text-gray-400 max-w-[130px]"><p class="truncate">{{ file.album_artist }}</p></td>
                    <td class="px-3 py-2 text-gray-600 dark:text-gray-400 max-w-[130px]"><p class="truncate">{{ file.album_title }}</p></td>
                    <td class="px-3 py-2 text-gray-500 dark:text-gray-400 text-xs max-w-[100px]"><p class="truncate">{{ file.genre }}</p></td>
                    <td class="px-2 py-2 text-center text-gray-400 text-xs">{{ file.year ?? '' }}</td>
                    <td class="px-3 py-2 text-gray-500 dark:text-gray-400 text-xs max-w-[120px]"><p class="truncate">{{ file.comment }}</p></td>
                    <td class="px-2 py-2 text-center">
                      <div class="flex flex-col items-center gap-0.5">
                        ...
                        <span v-if="file.is_cue_track" class="text-[9px] font-semibold px-1 py-0.5 rounded bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400">CUE</span>
                      </div>
                    </td>
                    <td class="px-2 py-2 text-center">
                    ...
                    <td class="px-2 py-2 text-center">
                      <div v-if="file.youtube_url" class="group/yt inline-flex items-center justify-center gap-0.5">
                        <button class="inline-flex items-center justify-center text-red-500 hover:text-red-600 opacity-70 hover:opacity-100 transition-opacity" :title="t('browser.ytWatch')" @click.stop="openYtDialog(file.youtube_url)">
                        ...
                        <button class="hidden group-hover/yt:flex items-center justify-center w-3.5 h-3.5 text-[9px] text-gray-300 hover:text-red-500 transition-colors leading-none" :title="t('browser.ytDeleteLink')" @click.stop="clearYoutubeUrl(file)">✕</button>
                      </div>
                    </td>
    ...
    <Teleport to="body">
      <div
        v-if="ytDialogVideoId"
        class="fixed inset-0 bg-black/80 z-[500] flex items-center justify-center p-4"
        @click.self="ytDialogVideoId = null"
      >
        <div class="relative w-full max-w-3xl">
          <button
            class="absolute -top-8 right-0 text-white/70 hover:text-white text-sm flex items-center gap-1"
            @click="ytDialogVideoId = null"
          >✕ {{ t('common.close') }}</button>
          <div class="relative w-full" style="padding-bottom: 56.25%;">
            <iframe
              :src="`https://www.youtube.com/embed/${ytDialogVideoId}?autoplay=1`"
...
const ytDialogVideoId = ref(null)
...
function _onKeyDown(e) {
  if (e.key === 'Escape' && ytDialogVideoId.value) {
    ytDialogVideoId.value = null
    e.stopPropagation()
  }
}
...
onMounted(async () => {
  ...
  document.addEventListener('click', onClickOutsideWizardMenu, true)
  document.addEventListener('keydown', _onKeyDown, true)
})
onUnmounted(() => {
  document.removeEventListener('click', onClickOutside, true)
  document.removeEventListener('click', onClickOutsideWizardMenu, true)
...
function onRowClick(file) {
  ...
  if (file.is_cue_track) return  // CUE 가상 트랙은 태그 편집 불가
...
async function exportFolderHtml() {
  ...
  try {
    ...
    // 파일 목록 새로고침 (그룹 모드면 재귀 로드, 아니면 일반 로드)
    await _reloadAfterJob(path)
  } catch (e) {
...
function openWizardWithPreset(preset) {
  showWizardMenu.value = false
  // 프리셋의 steps를 enabled된 것만 필터링하여 실행
  const presetSteps = preset.steps || []
  const resolved = []
  for (const s of presetSteps) {
    const def = WIZARD_DEFAULT_STEPS_BROWSER.find(d => d.id === s.id)
  ...
  onWizardStart(enabledSteps)
  browserStore.wizardOpen = true
}

function onClickOutsideWizardMenu(e) {
  if (wizardMenuRef.value && !wizardMenuRef.value.contains(e.target)) {
    showWizardMenu.value = false
  }
}
...
async function runWizard() {
  for (let i = 0; i < wizardSteps.value.length; i++) {
    ...
    if (step.id === 'autoTag') {
      ...
      const providerKeys = (step.providerKeys && step.providerKeys.length > 0)
        ? step.providerKeys
        : [availableProviders.value[0]?.key ?? 'spotify']
      selectedAutoProviders.value = providerKeys
...
async function _reloadAfterJob(folderPath) {
  browserStore.invalidateFilesCache(folderPath)
...
watch(() => jobStore.lrcJob?.done, async (done) => {
  ...
  if (job.ok > 0) {
    await _reloadAfterJob(job.folderPath)
  }
})

// YouTube 완료 감지: 파일 목록 갱신 + 토스트
watch(() => jobStore.youtubeJob?.done, async (done) => {
  if (!done) return
